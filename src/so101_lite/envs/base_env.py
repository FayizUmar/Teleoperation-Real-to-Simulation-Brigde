...
from so101_lite.camera_utils import compute_overhead_camera_params
...
from so101_lite.rewards import orientation_progress, reach_progress
...
class SO101NexusMuJoCoBaseEnv(gymnasium.Env):
    ...
    def _setup_camera_renderers(self) -> None:
        ...
        self._global_cam_component: GlobalCamera | None = None
        if self.config.observations is not None:
            for comp in self.config.observations:
                if isinstance(comp, WristCamera):
                    ...
                    self._global_cam_component = comp
        ...
        if self._global_cam_component is not None:
            from so101_lite.camera_utils import compute_angled_camera_params

            cam = self._global_cam_component
            params = compute_angled_camera_params(
                spawn_center=self.config.spawn_center,
                spawn_max_radius=self.config.spawn_max_radius,
                elevation=cam.elevation_deg,
    ...
    def _build_observation_space(self) -> spaces.Space:
        ...
        has_any_camera = (
            self._wrist_cam_component is not None
            or self._overhead_cam_component is not None
            or self._global_cam_component is not None
        )
        ...
        if self._global_cam_component is not None:
            gc = self._global_cam_component
            obs_dict["global_camera"] = spaces.Box(
                low=0,
                high=255,
                shape=(gc.height, gc.width, 3),
                dtype=np.uint8,
            )
    ...
    def _get_obs(self) -> np.ndarray | dict[str, np.ndarray]:
        ...
        has_any_camera = (
            ...
            or self._global_obs_renderer is not None
        )
        if not has_any_camera:
            return state
