...
from so101_lite.config import ControlMode, ReachConfig
...
def _build_reach_scene_xml(ground_rgba: list[float], target_radius: float, cameras_xml: str = "") -> str:
...
class ReachEnv(SO101NexusMuJoCoBaseEnv):
    ...
    def __init__(
        ...
        xml_string = _build_reach_scene_xml(ground_rgba, config.target_radius, cameras_xml=cameras_xml)
    ...
    def _get_component_data(self, component: object) -> np.ndarray:
        from so101_lite.observations import TargetOffset
