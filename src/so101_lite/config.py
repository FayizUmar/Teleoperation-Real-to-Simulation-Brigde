...
from so101_lite.objects import CubeObject, SceneObject
...
if TYPE_CHECKING:
    from so101_lite.observations import Observation
...
class EnvironmentConfig:
    ...
    def __init__(
        ...
        if self.obs_mode == "visual":
            from so101_lite.observations import _CameraObservation as _CamObs
        ...
        if self.observations is not None:
            from so101_lite.observations import _CameraObservation
...
class ChessBoardConfig(EnvironmentConfig):
    """Config for the chess board pick-and-place environment.

    The robot picks a chess piece from one board square and places it onto a
    highlighted target square. Two cameras capture the scene: a GlobalCamera
    for the full-board macro view and a WristCamera for close-up gripper control.

    Args:
        board_center: XY world position of the board centre (metres).
        square_size: Side length of each chess square (metres).
        board_thickness: Full thickness of the physical board (metres).
        piece_colors: Piece color name(s) to randomly sample each episode.
        target_colors: Target disc color name(s) to randomly sample each episode.
        source_colors: Source square marker disc color name(s) to randomly sample each episode.
        piece_radius: Radius of the cylindrical chess piece (metres).
        piece_half_height: Half-height of the cylindrical chess piece (metres).
        piece_mass: Mass of the chess piece in kg.
        home_thresh: Max TCP distance from its post-reset pose to count as
            "returned home" after placing the piece (metres).
        **kwargs: Forwarded to EnvironmentConfig.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs.setdefault("spawn_center", board_center)
        kwargs.setdefault("spawn_min_radius", 0.0)
        kwargs.setdefault("spawn_max_radius", board_half + 0.03)
        kwargs.setdefault("goal_thresh", square_size * 0.48)
        super().__init__(**kwargs)
    ...
    @property
    def board_half(self) -> float:
        """Half-extent of the board in metres."""
        return 4.0 * self.square_size

    @property
    def task_description(self) -> str:
        """Canonical task description derived from configured colors."""
        piece_name = (
            self.piece_colors if isinstance(self.piece_colors, str) else self.piece_colors[0]
        )
        return f"Pick up the {piece_name} chess piece and place it on the highlighted target square"

    def __repr__(self) -> str:  # noqa: D105
        return (
            f"ChessBoardConfig(board_center={self.board_center}, "
            f"square_size={self.square_size}, piece_colors={self.piece_colors!r})"
        )
